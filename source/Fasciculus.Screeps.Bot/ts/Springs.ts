...
export class Springs
{
    ...
    static get all(): Spring[] { return _.flatten(_.values<Room>(Game.rooms).map(Springs.allFrom)); }
    static get active(): Spring[] { return _.flatten(_.values<Room>(Game.rooms).map(Springs.activeFrom)); }
}